 ...
 * Created by LiuYi
 */
public class NYDragListView extends ListView {
    ...
    private NYDragListViewItem mDragItemInfo;
    ...
    public NYDragListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        setLayerType(View.LAYER_TYPE_HARDWARE, null);
        scaledTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
        mDragItemInfo = new NYDragListViewItem();
        init();
    }
...
//		}
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (ev.getAction() == MotionEvent.ACTION_DOWN && !isLock && !isMoving && !isDragItemMoving) {
            ...
            NYAbsDragListViewAdapter adapter = (NYAbsDragListViewAdapter) getAdapter();
            ...
            // 获取可拖拽的图标
            View draggerIcon = dragger.findViewById(R.id.item_image);
...
//		scaleAnimation.setDuration(300);
    ...
    private void hideDropItem() {
        final NYAbsDragListViewAdapter adapter = (NYAbsDragListViewAdapter) this.getAdapter();
...
//	private List<int[2]> listOrder = new ArrayList<Integer>();
    ...
    private void testAnimation(int y) {
        final NYAbsDragListViewAdapter adapter = (NYAbsDragListViewAdapter) getAdapter();
        int tempPosition = pointToPosition(0, y);
